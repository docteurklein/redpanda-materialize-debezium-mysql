...
create materialized view all_pim.product_value as
  with by_attr (code, rest) as (
    select j.* from all_pim.product, jsonb_each(raw_values) j
  ),
  by_channel (code, channel, rest) as (
    select code,
        case j.key when '<all_channels>' then null else j.key end,
        j.value
    from by_attr, jsonb_each(rest) j
  ),
  by_locale (code, channel, locale, value) as (
    select code, channel,
...
into kafka broker 'redpanda:9092' topic 'all_pim_product_value_edited'
format avro
using confluent schema registry 'http://redpanda:8081'
;
